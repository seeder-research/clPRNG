...
void ClPRNG::generateBufferKernel(std::string name, std::string type, std::string src) {
    ...
    switch(ClPRNG::LookupPRNG(name)) {
        case 20 :
            tmp += "\n"
                   "kernel void seed_prng(ulong seedVal, global " + name + "_state* state){\n"
        ...
        default :
            tmp += "\n"
                   "kernel void seed_prng(ulong seedVal, global " + name + "_state* stateBuf){\n"
                   "    uint gid=get_global_id(0);\n"
                   "    ulong seed = seedVal<<1;\n"
                   "    if (seed == 0) {\n"
                   "        seed += 1;\n"
...
cl_int ClPRNG::SeedGenerator() {
    ...
    cl_event event_id;
    cl::Event event;
    err = com_queue.enqueueNDRangeKernel(seed_rng, cl::NDRange(0), cl::NDRange((size_t)(wkgrp_count * wkgrp_size)), cl::NDRange((size_t)(wkgrp_size)), NULL, &event);
    if (err != 0) {
    ...
    if (local_state_mem == NULL) {
        static void *tmp_mem = malloc(state_size * (size_t)(wkgrp_size * wkgrp_count));
        local_state_mem = tmp_mem;
    } else {
        free(local_state_mem);
        static void *tmp_mem = malloc(state_size * (size_t)(wkgrp_size * wkgrp_count));
        local_state_mem = tmp_mem;
    }
    err = com_queue.enqueueReadBuffer(stateBuffer, true, 0, state_size * (size_t)(wkgrp_size * wkgrp_count), local_state_mem, NULL, &event);
    if (err != 0) {
...
// 1. Copy PRNG states from host side to device side
// 2. Generate random numbers and fill temporary buffer
// 3. Copy PRNG states from device side back to host side
cl_int ClPRNG::GenerateStream() {
    // Make sure stream object is correctly set up before generating random stream
    if (init_flag != true) {
        std::cout << "ERROR: stream object has not been initialized!" << std::endl;
        return -1;
    }
    ...
    cl::Event event;
    cl_int err = com_queue.enqueueWriteBuffer(stateBuffer, true, 0, state_size * (size_t)(wkgrp_count * wkgrp_size), local_state_mem, NULL, &event);
    if (err != 0) {
    ...
    err = com_queue.enqueueNDRangeKernel(generate_bitstream, cl::NDRange(0), cl::NDRange((size_t)(wkgrp_count * wkgrp_size)), cl::NDRange((size_t)(wkgrp_size)), NULL, &event);
    if (err != 0) {
    ...
    // Copy PRNG states from host side to device side
    err = com_queue.enqueueReadBuffer(stateBuffer, true, 0, state_size * (size_t)(wkgrp_count * wkgrp_size), local_state_mem, NULL, &event);
    if (err != 0) {
        std::cout << "ERROR: unable to copy state from host to device!" << std::endl;
        return err;
    }
